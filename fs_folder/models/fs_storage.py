...
class FsStorage(models.Model):
    ...
    def _check_fs_name_sanitization_replace_char(self):
        for rec in self:
            ...
            if rc and self._get_invalid_fs_name_chars_re_pattern().findall(rc):
    ...
    def _get_invalid_fs_name_chars(self) -> str:
        return r'<>:"/\\|?*\x00-\x1f'

    def _get_invalid_fs_name_chars_re_pattern(self) -> re.Pattern:
        pattern = f"[{self._get_invalid_fs_name_chars()}]"
        return re.compile(pattern)

    def is_fs_name_valid(self, name, raise_if_invalid=False) -> bool:
        self.ensure_one()
        invalid = self._get_invalid_fs_name_chars_re_pattern().findall(name)
    ...
    def sanitize_fs_item_name(self, name, replace_char=None) -> str:
        ...
        return (
            self._get_invalid_fs_name_chars_re_pattern()
            .sub(replace_char, name.strip())
            .strip()
        )
